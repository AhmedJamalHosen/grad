...
class DynamicUtilize(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.mac_to_port = {}
		self.vlan_config=  { '1' : [1,2,3,4,5,6] , '2' : [1,2,7,5] , '3' : [3,4,8,6] , '4' : [5,6,7,8] , '5' : [1,4,8,7] , '6' : [2,5,1,4] }
		self.datapaths = {}
    ...
    def add_flow(self, datapath, cookie, priority, match, actions, buffer_id=None):
		ofproto = datapath.ofproto
		parser = datapath.ofproto_parser
		idle_timeout = 40
		hard_timeout = 40
		...
		if buffer_id:
			mod = parser.OFPFlowMod(datapath=datapath,cookie=cookie,buffer_id=buffer_id,priority=priority,idle_timeout=idle_timeout,hard_timeout=hard_timeout, match=match,instructions=inst)
		else:
			mod = parser.OFPFlowMod(datapath=datapath,cookie=cookie,priority=priority,idle_timeout=idle_timeout,hard_timeout=hard_timeout, match=match, instructions=inst)
	...
	#part 1:normal operation: if the dst_mac is known,install a flow entry and forward it using pkt_out
		...
		if dpid == 7 or dpid == 8 or in_port in [1,2]:
			...
			if out_port != ofproto.OFPP_FLOOD:
				...
				if msg.buffer_id != ofproto.OFP_NO_BUFFER:
					...
					self.add_flow(datapath,cookie, 1, match, actions, msg.buffer_id)
					return
	    ...
	    # if packet come from port1 or port2 in (access) switches : flood to all port except the other

		if dpid != 7 and dpid != 8 and out_port == ofproto.OFPP_FLOOD:

			actions = []

	   	#flood to ports from 3 to 9 , that's enough for only 6 hosts or 6 vlans.
	...
	#part 2: install a flow with Vlan-id matching through the cookie (cookie = Vlan-id),then fowrward with a packet out to the root.
			...
			if vlan_id in self.root_table['1']:
				...
				match = parser.OFPMatch(in_port = in_port,vlan_vid=vlan_id|ofproto_v1_3.OFPVID_PRESENT)
    ...
    def _update_roots(self):
		...
		x = sorted(self.vlan_cost, key=self.vlan_cost.__getitem__, reverse=True)
		...
		for c in deleted_root:
			...
			for dp in self.datapaths.values():
				...
				if dp.id<7:
					...
					mod = parser.OFPFlowMod(datapath=datapath,cookie=cookie,cookie_mask=cookie_mask,command=ofproto.OFPFC_DELETE,out_port=ofproto.OFPP_ANY,out_group=ofproto.OFPG_ANY,match=match)
				...
				for dp in self.datapaths.values():
					...
					if dp.id < 7 :	
						...
						for i in [3,4,5,6,7,8,9,10]:
							...
							match = parser.OFPMatch(in_port = i,vlan_vid=x|ofproto_v1_3.OFPVID_PRESENT)
    ...
    def delete_flow(self, datapath, cookie, priority, match, output_port):
		...
		mod = parser.OFPFlowMod(datapath, command=ofproto.OFPFC_DELETE,out_port=output_port, out_group=ofproto.OFPG_ANY,priority=5, match=match)
    ...
    def send_aggregate_stats_request(self):
		...
		for dp in self.datapaths.values():
			...
			if dp.id == 7 or dp.id ==8:
				...
				req = ofp_parser.OFPAggregateStatsRequest(dp, 0,ofp.OFPTT_ALL,ofp.OFPP_ANY,ofp.OFPG_ANY,cookie, cookie_mask,match)
    ...
    def aggregate_stats_reply_handler(self, ev):
		...
		self.logger.debug('AggregateStats: packet_count=%d byte_count=%d ''flow_count=%d',body.packet_count, body.byte_count,body.flow_count)
